<script lang="ts">
  ...
  import { socket, logs, userCache, userData, selectedChannel, selectedGuild } from "$lib/stores";
  ...
  function dm(user_id: BigInt) {
    if ((user_id+"") === ($userData?.user.id+"")) createToast({ message: "Stop trying to dm yourself!", delay: 5000, type: "error" });
    // goto(`/@me/${user_id}`);
    const channel = $userData?.dms.find((c) => c.user1 === user_id || c.user2 === user_id);
    if (channel) {
