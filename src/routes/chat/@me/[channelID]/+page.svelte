<script lang="ts">
  import { DM_GUILD } from "$lib/constants";
  import { socket, logs, userCache, userData, selectedChannel, selectedGuild, fetched } from "$lib/stores";
  import { encode } from "cbor-x";
  import * as uuid from "uuid";
  import type { PageData } from "./$types"
	import { goto } from "$app/navigation";
  import { onMount } from "svelte";

  export let data: PageData;

  // console.log(data.channel);
  selectedChannel.set(data.channel);
  selectedGuild.set(DM_GUILD);
  // $: selectedChannel.update(_ => data.channel);
  $: msgs = $logs[$selectedChannel.id] || [];
  $: console.log("messages", msgs, $logs)
  $: console.log($userCache);
  $: console.log("DM: ", $userData?.dms)
  onMount(() => {
    // alert($selectedChannel.channel_type);
    if ($selectedChannel.channel_type !== 1) {
      alert("ok");
      goto("/chat");
    }
  });

  $: {
    if ($userData) {
      if (data.channel.channel_type === 1 && !$userData?.dms.some((c) => c.id === $selectedChannel.id)) {
        userData.update((usrd) => ({
          ...usrd!,
          dms: [...usrd!.dms, data.channel]
        }));
      }
    }
  }

  function try_user(id: bigint) {
    if (!$fetched.users.includes(id)) {
      fetched.update((fetched) => ({
        ...fetched,
        users: [...fetched.users, id]
      }));
      $socket?.sendQueued(
        encode({
          type: "UserFetch",
          data: {
            id,
          },
        })
      );
    }
  }
</script>

{#if $userData && $userData.user.id}
  {#each msgs as i}
    {#if $userCache.hasOwnProperty(i.author.id + "")}
      <div
        class={(i.author.id == $userData?.user.id ? (i.id !== "NOT_RECEIVED" ? "" : "text-gray-500") : "") + " flex flex-row my-4"}
        on:dblclick={() => {
          if (
            i.author.id != BigInt($userData?.user.id || 0)
          )
            return;
          const inp = prompt("New content");
          if (!inp) return;
          $socket?.send(
            encode({
              type: "MessageUpdate",
              data: {
                id: uuid.parse(i.id),
                content: inp,
                nonce: uuid.parse(i.nonce || DM_GUILD.id),
              },
            })
          );
        }}
      >
        <img class="w-8 h-8 inline-block m-2" alt="pfp" src={$userCache[i.author.id + ""]?.profile || "data:image/jpeg;base64,"}>
        <section class="text-black dark:text-[#f3f4f6]">
            {$userCache[i.author.id + ""]?.username}:
            {#each i.content.trim().split('\n') as line}
              <p class="before:content-[''] before:inline-block before:w-0">{line}</p>
            {/each}
            {i.created_at !== i.edited_at ? "(edited)" : ""}
        </section>
      </div>
    {:else if [try_user(i.author.id), true][1]}
      <div class="flex flex-row my-4">
        <span class="w-8 h-8 inline-block m-2 bg-gray-300 rounded-md animate-pulse" />
        <section class="w-full flex flex-col p-2">
          <p class="mb-1 w-full h-[14px] before:content-[''] before:inline-block before:w-0 bg-gray-300 animate-pulse rounded-md" />
          <p class="w-full h-[14px] before:content-[''] before:inline-block before:w-0 bg-gray-300 animate-pulse rounded-md" />
        </section>
      </div>
    {/if}
  {/each}
{:else}
  <div class="fixed z-90 w-screen h-screen flex justify-center items-center bg-gray-900 top-0 left-0">
    <div class="flex flex-col text-white text-center text-xl font-light space-y-3">
      LOADING
    </div>
  </div>
{/if}