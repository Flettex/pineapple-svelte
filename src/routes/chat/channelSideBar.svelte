<script lang="ts">
  ...
  import { selectedChannel, selectedGuild, userCache, userData } from "$lib/stores";
...
<div class="w-60 h-full m-0 ml-16 bg-[#F2F3F5] dark:bg-[#2b2d31]">
  ...
  <div class="flex flex-col items-center justify-start p-1 m-0 h-[90%] overflow-y-auto">
    {#each $selectedGuild.channels || $userData?.dms || [] as c (c.id)}
      <button
      ...
      >
        ...
        <p class="ml-1">{c.channel_type === 1 ? $userCache[((c.user1+"") === ($userData?.user.id + "") ? c.user2 : c.user1)+""]?.username : c.name}</p>
      </button>
    {/each}
  </div>
